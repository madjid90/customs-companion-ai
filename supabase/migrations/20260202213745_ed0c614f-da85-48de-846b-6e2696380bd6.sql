-- ============================================
-- FONCTIONS RPC HYBRIDES AVEC ALGORITHME RRF
-- Utilise extensions.vector explicitement
-- ============================================
...
BEGIN
  ...
  WITH semantic_results AS (
    SELECT 
      ...
      hs.chapter_number,
      hs.level,
      1 - (hs.embedding OPERATOR(extensions.<=>) embedding_vector) as sem_score,
      ROW_NUMBER() OVER (ORDER BY hs.embedding OPERATOR(extensions.<=>) embedding_vector) as sem_rank
    FROM public.hs_codes hs
    WHERE 
      hs.embedding IS NOT NULL
      AND hs.is_active = true
    ORDER BY hs.embedding OPERATOR(extensions.<=>) embedding_vector
    LIMIT 50
  ),
  fts_results AS (
    SELECT 
...
BEGIN
  ...
  WITH semantic_results AS (
    SELECT 
      ...
      tn.anchor,
      tn.page_number,
      1 - (tn.embedding OPERATOR(extensions.<=>) embedding_vector) as sem_score,
      ROW_NUMBER() OVER (ORDER BY tn.embedding OPERATOR(extensions.<=>) embedding_vector) as sem_rank
    FROM public.tariff_notes tn
    WHERE 
      tn.embedding IS NOT NULL
      AND (chapter_filters IS NULL OR tn.chapter_number = ANY(chapter_filters))
    ORDER BY tn.embedding OPERATOR(extensions.<=>) embedding_vector
    LIMIT 30
  ),
  fts_results AS (
    SELECT 
