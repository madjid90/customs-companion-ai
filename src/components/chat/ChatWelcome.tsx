...
export function ChatWelcome({ onQuestionClick }: ChatWelcomeProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-end md:justify-center min-h-0 h-full max-h-full px-3 md:px-6 py-2 pb-2 md:py-6 animate-fade-in overflow-hidden">
      {/* Robot icon - hidden on small mobile to save space */}
      <div className="relative mb-2 md:mb-5 flex-shrink-0 hidden sm:block">
      ...
      {/* Title block */}
      <h2 className="text-lg md:text-2xl font-extrabold text-foreground mb-0.5 md:mb-1 tracking-tight text-center flex-shrink-0">
      ...
      {/* Suggestion cards */}
      <div className="w-full max-w-xl space-y-1.5 md:space-y-0 md:bg-card md:rounded-2xl md:border md:border-border md:shadow-md md:overflow-hidden flex-shrink-0">
        {suggestedQuestions.map((item, i) => (
          <button
            key={`${item.question}-${i}`}
            className={cn(
              "group flex items-center gap-3 md:gap-4 text-left w-full px-3 py-2 md:px-5 md:py-3.5 rounded-xl md:rounded-none bg-card md:bg-transparent border border-border/50 md:border-0 shadow-sm md:shadow-none hover:bg-muted/50 transition-all duration-200",
              i < suggestedQuestions.length - 1 && "md:border-b md:border-border"
            )}
            onClick={() => onQuestionClick(item.question)}
          >
            <div className="flex-shrink-0 w-8 h-8 md:w-10 md:h-10 rounded-xl bg-primary/10 flex items-center justify-center group-hover:bg-primary/15 transition-colors">
              <item.icon className="h-3.5 w-3.5 md:h-[18px] md:w-[18px] text-primary" />
            </div>
